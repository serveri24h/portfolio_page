...
<logo>
    <svg width="400" height="400" viewBox="0 0 400 500">
        ...
        {#each [0,1,2] as i}
            ...
            {#each [0,1,2,3] as bx}
                {#each [0,1,2] as by}
                    <rect x="{x_offset+button_offset+bx*button_distance}" y="{y_offset+button_offset+box_distance*i+button_distance*by}" width="{button_w}" height="{button_h}" class="serv-button"/>
                {/each}
            {/each}
